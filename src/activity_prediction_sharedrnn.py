...
def predict_activitiy(index,fold,checkpoint,architecture='joint'):
	...
	# path_to_checkpoints = '/scr/ashesh/activity-anticipation/checkpoints1/{0}'.format(fold)

	#path_to_checkpoints = '/media/bsb/666/human_anticipation_data/CAD_120/features_cad120_ground_truth_segmentation/checkpoints/{0}'.format(fold)
	path_to_checkpoints = '/media/bsb/666/human_anticipation_data/CAD_120/features_cad120_ground_truth_segmentation/checkpoints/fold_1'
	path_to_result = '/media/bsb/666/human_anticipation_data/result/'
	path_to_dataset = '/media/bsb/666/human_anticipation_data/CAD_120/features_cad120_ground_truth_segmentation/dataset/{0}'.format(fold)
	...
	if architecture == 'detection':
		for xte_shared,xte,yte in zip(X_te_human_shared,X_te_human_disjoint,Y_te_human):
			...
			human_label.append(prediction)
			t = np.nonzero(yte-prediction)
			errors += len(t[0])
			N += yte.shape[0]
		cPickle.dump(human_label, open('{1}/{2}_human_{0}.pik'.format(index, path_to_result, architecture), 'wb'))
		del human_label
		...
		for xte_shared,xte,yte in zip(X_te_objects_shared,X_te_objects_disjoint,Y_te_objects):
			...
			predictions.append(prediction)
			object_label.append(prediction)
			t = np.nonzero(yte-prediction)
			errors_objects += len(t[0])
			N_objects += yte.shape[0]
		cPickle.dump(object_label, open('{1}/{2}_object_{0}.pik'.format(index, path_to_result, architecture), 'wb'))
		del object_label
...
if __name__ == "__main__":
	...
	result = predict_activitiy(index,'fold_1',checkpoint,architecture)
	detection = result['detection']
	anticipation = result['anticipation']
	if detection.shape[0] > 0:
		err_detection.append(detection)
