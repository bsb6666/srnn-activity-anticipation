...
'''
def jointModelOutput(num_sub_activities, num_affordances, num_sub_activities_anticipation, num_affordances_anticipation, inputJointFeatures, inputHumanFeatures, inputObjectFeatures):
...
if __name__ == '__main__':
	...
	main_path = ''
		...
		#main_path = '.'
        if soc.gethostname() == "bsb-CW65S":
		main_path = '/media/bsb/666/human_anticipation_data/CAD_120/features_cad120_ground_truth_segmentation'	
	else:
	...
	if not use_pretrained:
		...
		if architectures[train_for] == 'detection':
			...
			rnn = jointModel(num_sub_activities, num_affordances, inputJointFeatures,inputHumanFeatures, inputObjectFeatures )
			rnn.fitModel(X_tr_human_shared, X_tr_human_disjoint, Y_tr_human, X_tr_objects_shared, X_tr_objects_disjoint, Y_tr_objects, 1, '{1}/{0}/'.format(index,path_to_checkpoints), epochs,batch_size, learning_rate_decay, decay_after)

		elif architectures[train_for] == 'anticipation':

 			rnn = jointModel(num_sub_activities_anticipation, num_affordances_anticipation,inputJointFeatures, inputHumanFeatures, inputObjectFeatures)
			rnn.fitModel(X_tr_human_shared, X_tr_human_disjoint, Y_tr_human_anticipation,X_tr_objects_shared, X_tr_objects_disjoint, Y_tr_objects_anticipation, 1,'{1}/{0}/'.format(index,path_to_checkpoints), epochs, batch_size, learning_rate_decay, decay_after)

		elif architectures[train_for] == 'joint':

			rnn = jointModelOutput(num_sub_activities, num_affordances, num_sub_activities_anticipation,num_affordances_anticipation, inputJointFeatures, inputHumanFeatures, inputObjectFeatures)
			rnn.fitModel(X_tr_human_shared, X_tr_human_disjoint, Y_tr_human, Y_tr_human_anticipation,X_tr_objects_shared, X_tr_objects_disjoint, Y_tr_objects, Y_tr_objects_anticipation, 1,'{1}/{0}/'.format(index,path_to_checkpoints), epochs, batch_size, learning_rate_decay, decay_after)
		else:
